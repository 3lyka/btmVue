...
const mutations = {
    ...
    setSwitchAccepted(state, payload) {
        state.offerList = state.offerList.filter(offer => {
            if(offer.id === payload.offerId) {
                ...
                if(payload.switchAccepted === false && payload.isTrader && offer.status.code === 'new') {
                    offer.weightSale = 0
                }
                if(payload.switchAccepted === true && payload.weightSale === 0 && payload.isTrader && offer.status.code === 'new') {      
    ...
    setOffersList(state, payload, ) {
        ...
        state.offerList = state.offerList.filter(offer => {
            if(offer.isAccepted === true && !offer.weightSale) {
                offer.switchAccepted = true
                offer.weightSale = offer.weight
                return true
            } else if(offer.isAccepted === false && offer.weightSale && offer.status.code !== 'umiop_check' && offer.status.code !== 'umiop_done' && offer.status.code !== 'umiop_canceled') {
                offer.weightSale = 0
                return true
            } else if(offer.isAccepted === true && offer.weightSale) {
                offer.switchAccepted = true
                return true
            } else {
